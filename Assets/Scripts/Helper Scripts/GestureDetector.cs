...
namespace SpellBind
{
    ...
    public class GestureDetector : MonoBehaviour
    {
        ...
        void Update()
        {
            // if in debug mode and we press S, we will save a gesture
            if (debugMode && Input.GetKeyDown(KeyCode.S))
            {
                // Call the function for saving the gesture
                Save();
            }

            //if the initialization was successful
            if (hasStarted.Equals(true) && !debugMode)
            {
        ...
        void Save()
        {
            Debug.LogFormat("<color=green>SAVE GESTURE {0}!!</color>",fingerbones.Count);

            // We create a new Gesture struct
            Gesture g = new Gesture();

            // givin to it a default name
            g.name = "Defense Gesture";
